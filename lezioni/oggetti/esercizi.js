...
let mioGarage = {
    ...
    aggiungoAutomobile: function(modello, marchio) {
      this.automobili.push({
        model: modello,
        brand: marchio
      })
    },

    trovaAutomobiliPerBrand: function(marchio) {
      let listaPerBrand = this.automobili.filter(function(automobile) {
        return automobile.brand === marchio
      })
    }
}


mioGarage.trovaAutomobiliPerBrand("lancia")
...
// calcolo i totali
for (let k in bowling) {
  let giocatoreSingolo = bowling[k];

  giocatoreSingolo.totale = giocatoreSingolo.punteggi.reduce(function(totale, punteggio) {
    return totale += punteggio
  })
}


let giocatoreConPunteggioMaggiore = {
  totale: 0
};

let punteggioMaggiore = 0;

for (let k in bowling) {
  let giocatoreSingolo = bowling[k];

  if (giocatoreSingolo.totale > giocatoreConPunteggioMaggiore.totale) {
    giocatoreConPunteggioMaggiore = giocatoreSingolo
  }
}