...
function titolo(){
    ...
    .then(function(listaPost){
        listaPost.forEach(function(dati){
            ...
            .then(function(lista){
                let ciao = document.getElementById(`elemento${dati.id}`)
                ciao.innerHTML= `<strong>Name:</strong> <a href="https://jsonplaceholder.typicode.com/posts/?userId=${dati.userId}&" target="_blank"> ${lista.name}</a> <br> <strong>Username:</strong> ${lista.username} <br> <strong>Mail:</strong> ${lista.email} <br> <strong> Phone: </strong> ${lista.phone} <br> <strong>Website: </strong> ${lista.website}`
            })
            })
        })
    })
